---
title: "React の Container/Presentational × Hooksを関数型アーキテクチャで再設計する"
emoji: "🛠️"
type: "tech"
topics:
  - "react"
...
本記事のアプローチは、Container/Presentationalパターンに「Functional Core, Imperative Shell」の考え方を組み合わせ、**副作用の境界**を設計の軸に据えるものです。単なる責務分離ではなく、「どこに何を書くべきか」がレイヤーの性質から決まる構造を目指します。
...
本文で段階的に改善していきますが、基本形のContainerは次のような形になります。
...
なお、ここでは説明の便宜上HooksとContainerを分けて示していますが、実際にはイベントハンドラの形式（`ChangeEvent`を受け取るか、`SortKey`を受け取るか）はPresentationalのprops設計に依存するため、Hookではなく利用側のContainerが決める方が柔軟です。
...
const baseUsers: User[] = [
  { id: "1", name: "Alice", createdAt: "2024-03-01" },
...
propsだけに依存するため、任意のpropsを渡してUIの出力を検証できます。APIのモックやProviderのラップは不要です。Storybookとの相性も良く、ビジュアルリグレッションテストの対象としても扱いやすくなります。
...
describe("UserListPresentational", () => {
  it("ユーザー一覧が表示される", () => {
    render(
      <UserListPresentational
        users={[{ id: "1", name: "Alice", createdAt: "2024-01-15" }]}
        sortKey="name"
...
ここまでで、概要で示した基本形のContainerが完成しました。ここからは、この構造をさらに発展させるパターンを紹介します。
...
ここがこの設計のポイントです。`handleChangeCompareFn`は比較関数の更新だけでなく、ページのリセットも行っています。ソート変更時にページを1に戻す——この「子Container間の協調」は、親Containerだからこそ担える責務です。`UserListSortContainer`はページネーションの存在を知らず、`UserListPaginationContainer`はソートの存在を知りません。各子Containerは自身の関心に閉じたまま、親Containerが全体を協調させています。
...
features/
  users/
    ...
    user-list/
      ...
      user-list-sort/
        UserListSortContainer.tsx
        UserListSortPresentational.tsx
        userSortOptions.ts        # 関数型コア
      user-list-pagination/
        UserListPaginationContainer.tsx
        UserListPaginationPresentational.tsx
    user-search-dialog/
